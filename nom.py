...
def nominees_helper(awards_and_winners,data):
	...
	checkfor = ["beat", "beats", "beating", "better than", "upset", "liked", "wish", "robbed"]
	final_dict = {}
	final_dict2 = {}
	...
	if awards_and_winners:
		for tweet_json in data:
			...
			for checkword in checkfor:
				if checkword in tweet_lower and value in tweet_lower and value != "":
					# print(tweet)
					# print ("hi hi")
...
def nominees(data, award_list, winners_dict):
	...
	for award in award_list:
		if award in winners_dict:
			award_winner = winners_dict[award]
			award_winner_pair = {award:award_winner}
			list_of_nominees = nominees_helper(award_winner_pair, data)
			nom_dict.update({award:list_of_nominees})
		else:
